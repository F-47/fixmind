...
This runs Fixmind without a global install. It detects Claude Code, Cursor, and Codex, then registers the `fixmind` MCP server for the clients you choose, either for the whole machine or for a single project with `--scope project`. It also initializes Fixmind's local data and config files. Restart your AI client after setup finishes. Full flag reference: [Commands](cli-reference.md#fixmind-setup). Need client-specific setup steps for VS Code, Copilot CLI, OpenCode, Pi, or another MCP client? See [MCP Clients](mcp-clients.md).
...
With the CLI installed globally, you can run commands like `fixmind setup`, `fixmind review`, and `fixmind status` directly.
...
- [MCP Integration](mcp-integration.md) - how the MCP server and tool actually work, token overhead, manual client config.
