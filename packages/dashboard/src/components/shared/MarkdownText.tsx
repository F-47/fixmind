    ...
    <div className={cn(className)}>
      <ReactMarkdown
        components={{
          ...
          ol: ({ children: content }) => (
            <ol className="mt-3 list-decimal space-y-1 pl-5">{content}</ol>
          ),
          li: ({ children: content }) => <li>{content}</li>,
          blockquote: ({ children: content }) => (
            <blockquote className="mt-3 border-l-2 border-line pl-4 text-muted italic">
              {content}
            </blockquote>
          ),
          ...
          pre: ({ children: content }) => (
            <pre className="mt-3 overflow-x-auto border border-line bg-surface p-4 font-mono text-sm leading-relaxed">
              {content}
            </pre>
          ),
            ...
            return (
              <code
                className={cn(
                  isBlock
                    ? codeClassName
                    : "rounded bg-surface px-1.5 py-0.5 font-mono text-[0.9em]",
                )}
              >
                {content}
              </code>
