        ...
        <div className="flex flex-col gap-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex min-w-0 items-start gap-3">
              <div className={cn("mt-0.5 flex size-11 shrink-0 items-center justify-center rounded-2xl border", presentation.iconClass)}>
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <Metric label="Account" value={syncMeta?.email ?? (syncMeta?.loggedIn ? "Signed in" : "Local only")} />
      ...
      >
        <div className="border border-line/80 bg-[radial-gradient(circle_at_top_right,rgba(124,92,255,0.18),transparent_34%),linear-gradient(180deg,rgba(17,20,27,0.98),rgba(11,13,19,0.98))] p-6">
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-2">
            ...
            <DetailCard
              title="Next action"
              lines={nextActions(syncMeta, refreshing)}
            />
          </div>
        </div>
      </dialog>
  ...
  switch (state) {
    case "loading":
      return {
        ...
        icon: RefreshCw,
        cardClass: "border-line bg-[linear-gradient(135deg,rgba(22,26,35,0.98),rgba(10,12,18,0.98))]",
    ...
    case "syncing":
      return {
        ...
        description: "Fixmind is pushing local lesson changes first, then pulling the latest remote updates back into this dashboard.",
        detailsLabel: "Syncing now",
        actionLabel: "Sync now",
        icon: RefreshCw,
        cardClass: "border-accent/30 bg-[radial-gradient(circle_at_top_right,rgba(124,92,255,0.2),transparent_32%),linear-gradient(135deg,rgba(22,26,35,0.98),rgba(10,12,18,0.98))]",
    ...
    case "needs_reauth":
      return {
        ...
        description: "Your saved sync session expired. Lessons stay local, but this device cannot sync again until you sign in here one more time.",
        detailsLabel: "Needs re-login",
        actionLabel: "Retry status",
        icon: ShieldAlert,
        cardClass: "border-warn/25 bg-[radial-gradient(circle_at_top_right,rgba(232,179,95,0.18),transparent_34%),linear-gradient(135deg,rgba(22,26,35,0.98),rgba(10,12,18,0.98))]",
    ...
    case "conflicts":
      return {
        ...
        description: "This device detected lesson changes that should be reviewed before a final sync choice is made.",
        detailsLabel: "Conflicts pending",
        actionLabel: "Sync now",
        icon: AlertTriangle,
        cardClass: "border-danger/25 bg-[radial-gradient(circle_at_top_right,rgba(255,92,114,0.14),transparent_34%),linear-gradient(135deg,rgba(22,26,35,0.98),rgba(10,12,18,0.98))]",
    ...
    case "unsynced_changes":
      return {
        ...
        actionLabel: "Sync now",
        icon: AlertTriangle,
        cardClass: "border-accent/30 bg-[radial-gradient(circle_at_top_right,rgba(124,92,255,0.16),transparent_34%),linear-gradient(135deg,rgba(22,26,35,0.98),rgba(10,12,18,0.98))]",
        iconClass: "border-accent/30 bg-accent/10 text-accent",
      };
    case "up_to_date":
      return {
        ...
        actionLabel: "Sync now",
        icon: ShieldCheck,
        cardClass: "border-positive/25 bg-[radial-gradient(circle_at_top_right,rgba(74,222,128,0.16),transparent_34%),linear-gradient(135deg,rgba(22,26,35,0.98),rgba(10,12,18,0.98))]",
    ...
    case "unavailable":
      return {
        ...
        description: "Fixmind could not confirm sync health for this device right now, or this account does not currently have encrypted sync active.",
...
function humanizeError(message: string): string {
  if (/Incorrect passphrase/i.test(message)) return "Another device used a different passphrase. Reconnect with the same sync passphrase.";
  if (/active Pro or Team plan/i.test(message)) return "This account does not currently have sync access.";
  if (/timed out/i.test(message)) return "The sync request timed out before the server responded.";
  if (/invalid refresh token|refresh token not found|jwt expired|session not found/i.test(message)) {
...
function nextActions(syncMeta: SyncMeta | null, refreshing: boolean): string[] {
  if (refreshing) return ["Wait for the current sync cycle to finish. The dashboard is uploading local lessons, then checking for remote updates."];
  if (!syncMeta || !syncMeta.loggedIn) return ["Continue in local-only mode, or reconnect this device when you want encrypted sync available in the dashboard."];
  if (syncMeta.needsReauth) return ["Reconnect this device to restore encrypted sync, then use Retry status here to confirm the session is healthy again."];
  if (!syncMeta.syncEnabled) return ["Use Retry status to check this device again after your account or connection issue is resolved."];
  if (syncMeta.pendingPushCount > 0) return ["Use Sync now to upload the pending local lesson changes and then pull back the latest remote state."];
  return ["No action needed right now. This device is connected and the lesson store looks current."];
}
