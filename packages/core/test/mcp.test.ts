  ...
  try {
    ...
    const result = await client.callTool({
      ...
      arguments: {
        ...
        codeExplanation:
          "The corrected version waits until the component is running in the browser.",
...
test("MCP memory returns reviewed active lessons and skips superseded ones", async () => {
  ...
  const oldLesson = store.save(
    memoryLesson("Hydration mismatch", "Keep the first server and browser render identical."),
  );
  store.updateReview(oldLesson.id, oldLesson.reviewQuestions, "understood");
  const newLesson = store.save(
    memoryLesson("Hydration fix", "Read browser-only state after mount."),
  );
  ...
  const unrelated = store.save(
    memoryLesson("Stale closure", "Use the latest value in the effect."),
  );
  ...
  try {
    ...
    const result = await client.callTool({
      ...
      arguments: {
        ...
        takeaway:
          "A static zod default always wins over runtime data unless the field is left unset.",
        whenNotApplicable:
          "Does not apply when the tool name is supplied explicitly by the caller.",
        ...
        reviewQuestions: [
          {
            ...
            expectedAnswer:
              "From the MCP client's clientInfo.name sent during the initialize handshake.",
          },
        ],
      },
    });
  ...
  try {
    ...
    const result = await client.callTool({
      ...
      arguments: {
        ...
        fixSummary:
          "Read localStorage after hydration so the server and browser render the same initial markup",
        ...
        reviewQuestions: [
          {
            ...
            expectedAnswer:
              "Render a stable initial state first, then read browser-only state after hydration.",
          },
        ],
        understanding: "unknown",
      },
    ...
    assert.equal(
      result.isError,
      undefined,
      result.isError ? (result.content as Array<{ text: string }>)[0].text : undefined,
    );
  ...
  try {
    ...
    const result = await client.callTool({
      ...
      arguments: {
        ...
        whenNotApplicable:
          "Does not apply to helpers used by only one file - inlining is fine there.",
        concepts: ["single-responsibility"],
        reviewQuestions: [
          {
            question: "When should you extract helpers?",
            expectedAnswer: "When they are reused or obscure intent.",
          },
        ],
      },
    });
  ...
  try {
    ...
    const result = await client.callTool({
      ...
      arguments: {
        ...
        rootCause:
          "The save handler trusted the form payload instead of validating required fields on the server",
        fixSummary:
          "Added a server-side check that rejects profiles missing an email before saving",
        ...
        reviewQuestions: [
          {
            question: "Where else should a required-field check like this live?",
            expectedAnswer:
              "On the server, for any field the UI marks required, since the UI check can be bypassed.",
          },
        ],
      },
    });
  ...
  try {
    ...
    const result = await client.callTool({
      ...
      arguments: {
        ...
        rootCause:
          "The button's className hardcoded bg-[#2563eb] instead of the bg-primary utility class",
        ...
        whenNotApplicable:
          "Does not apply once a project has no shared design-system tokens at all.",
        concepts: ["styling"],
        reviewQuestions: [
          {
            question: "What did you change?",
            expectedAnswer: "Swapped a hardcoded hex color for a token.",
          },
        ],
      },
    });
