...
- Hosted sync enforces an active, unexpired entitlement in database policy as well as in the client, so removing the client check does not grant sync access. Subscriptions without an expiry remain valid while active.
