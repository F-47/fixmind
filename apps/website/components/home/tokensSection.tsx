...
function SessionTimeline() {
  return (
    <div className="rounded-xl border border-line bg-surface p-6">
      <p className="font-mono text-xs uppercase tracking-[0.15em] text-muted">A typical session</p>
      ...
      <p className="mt-8 border-t border-line pt-4 text-sm leading-relaxed text-muted">
        Everything in between - the turns spent reading code, writing patches, running tests - calls{" "}
        <code className="text-ink">save_lesson</code> zero times. Later, the agent can call{" "}
        <code className="text-ink">memory</code> to pull a few reviewed lessons back into context
        when the next task looks familiar.
      </p>
    </div>
  );
}
  ...
  return (
    <section id="tokens" className="border-y border-line bg-surface/40">
      <div className="mx-auto max-w-6xl px-6 py-24">
        <p className="font-mono text-xs uppercase tracking-[0.2em] text-accent">Token cost</p>
        ...
        <p className="mt-4 max-w-lg text-muted">
          Connecting an MCP server isn&apos;t free context-wise. Here&apos;s exactly where fixmind
          spends it - and where it doesn&apos;t.
        </p>
        <div className="mt-14 grid gap-16 lg:grid-cols-[1fr_420px] lg:items-start">
          <div>
            <TokenStep number="01" title="Sent once per session (~2,500 tokens, average)">
              The server&apos;s instructions and the <code className="text-ink">save_lesson</code>{" "}
              schema go out when the client connects. Clients that support prompt caching reuse that
              across every later turn, dropping the cost to{" "}
            ...
            <TokenStep number="02" title="Silent on every other turn (0 tokens)">
              The tool is only called when the agent decides a fix actually taught it something -
              never on a whim, never on formatting or renames.
            </TokenStep>
            <TokenStep number="03" title="A commit-message-sized payload (~500 tokens, average)">
              When a lesson is saved, the problem, root cause, fix summary, and code examples
              together are about as big as a short commit message or review comment - costing{" "}
